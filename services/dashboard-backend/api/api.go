...
func HandleCreateNewUser(ctx context.Context, dbQueries *db2.Queries, firebaseId string) (*HandleDashboardUserPostLoginDTO, error) {
	...
	project, createProjectErr := dbQueries.CreateProject(ctx, db2.CreateProjectParams{Name: "Default Project", Description: "Default Project"})
	if createProjectErr != nil {
		log.Error().Err(createProjectErr).Msg("failed to create project row")
		return nil, createProjectErr
	}

	userProject, createUserProjectErr := dbQueries.CreateUserProject(ctx, db2.CreateUserProjectParams{
		UserID:               user.ID,
		ProjectID:            project.ID,
		Permission:           db2.AccessPermissionTypeADMIN,
	...
	var userProjects = []db2.FindProjectsByUserIdRow{{
		ID:                   project.ID,
...
func HandleRetrieveUserProjects(ctx context.Context, dbQueries *db2.Queries, firebaseId string) (*HandleDashboardUserPostLoginDTO, error) {
