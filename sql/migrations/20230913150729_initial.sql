...
CREATE TABLE "application" ("id" uuid NOT NULL DEFAULT gen_random_uuid(), "app_id" text NOT NULL, "description" text NOT NULL, "name" character varying(256) NOT NULL, "public_key" text NOT NULL, "project_id" uuid NOT NULL, "created_at" timestamptz NOT NULL DEFAULT now(), "updated_at" timestamptz NOT NULL DEFAULT now(), PRIMARY KEY ("id"), CONSTRAINT "application_project_id_fkey" FOREIGN KEY ("project_id") REFERENCES "project" ("id") ON UPDATE NO ACTION ON DELETE CASCADE);
-- Create index "idx_app_id_project_id" to table: "application"
CREATE UNIQUE INDEX "idx_app_id_project_id" ON "application" ("app_id", "project_id");
-- Create "prompt_config" table
CREATE TABLE "prompt_config" ("id" uuid NOT NULL DEFAULT gen_random_uuid(), "model_type" "model_type" NOT NULL, "model_vendor" "model_vendor" NOT NULL, "model_parameters" json NOT NULL, "prompt_template" json NOT NULL, "template_variables" json NULL, "created_at" timestamptz NOT NULL DEFAULT now(), "updated_at" timestamptz NOT NULL DEFAULT now(), PRIMARY KEY ("id"));
-- Create "application_prompt_config" table
CREATE TABLE "application_prompt_config" ("application_id" uuid NOT NULL, "prompt_config_id" uuid NOT NULL, "version" serial NOT NULL, "is_latest" boolean NOT NULL DEFAULT false, PRIMARY KEY ("application_id", "prompt_config_id"), CONSTRAINT "application_prompt_config_application_id_fkey" FOREIGN KEY ("application_id") REFERENCES "application" ("id") ON UPDATE NO ACTION ON DELETE CASCADE, CONSTRAINT "application_prompt_config_prompt_config_id_fkey" FOREIGN KEY ("prompt_config_id") REFERENCES "prompt_config" ("id") ON UPDATE NO ACTION ON DELETE CASCADE);
-- Create "user" table
